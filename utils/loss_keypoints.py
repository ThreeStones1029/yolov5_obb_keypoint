...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        tcls, tbox, indices, anchors, tgaussian_theta, tlandmarks, lmks_mask = self.build_targets(p, targets)  # targets
        ...
        lmark *= self.hyp['keypoint'] # 增加关键点损失
        bs = tobj.shape[0]  # batch size

        # return (lbox + lobj + lcls) * bs, torch.cat((lbox, lobj, lcls)).detach()
        # return (lbox + lobj + lcls + ltheta) * bs, torch.cat((lbox, lobj, lcls, ltheta)).detach()
        return (lbox + lobj + lcls + ltheta + lmark) * bs, torch.cat((lbox, lobj, lcls, ltheta, lmark)).detach()
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            t[:, :, 7:9] /= self.stride[i] # keypoints featuremap pixel
            if nt:
                # Matches
        ...
        return tcls, tbox, indices, anch, tgaussian_theta, landmarks, lmks_mask
