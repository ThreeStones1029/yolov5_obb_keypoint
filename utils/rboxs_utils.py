...
def rbox_and_points2poly_and_points(obboxes_and_points):
    """
    ...
    """
    obboxes = obboxes_and_points[:, :5]
    keypoints = obboxes_and_points[:, 5:]
    if isinstance(obboxes, torch.Tensor):
        center, w, h, theta = obboxes[:, :2], obboxes[:, 2:3], obboxes[:, 3:4], obboxes[:, 4:5]
        Cos, Sin = torch.cos(theta), torch.sin(theta)
        ...
        point3 = center - vector1 - vector2
        point4 = center - vector1 + vector2 
        order = obboxes_and_points.shape[:-1]
        return torch.cat(
            (point1, point2, point3, point4, keypoints), dim=-1).reshape(*order, 10)
    else:
        center, w, h, theta = np.split(obboxes, (2, 3, 4), axis=-1)
        Cos, Sin = np.cos(theta), np.sin(theta)
        ...
        order = obboxes_and_points.shape[:-1]
        return np.concatenate(
            [point1, point2, point3, point4, keypoints], axis=-1).reshape(*order, 10)
...
def poly_and_keypoint_filter(polys_and_keypoints, h, w):
    """
    ...
    Return：
        keep_masks (array): (num)
    """
    x = polys_and_keypoints[:, 0::2]  # 提取所有 x 坐标 (num, 5)
    y = polys_and_keypoints[:, 1::2]  # 提取所有 y 坐标 (num, 5)

    # 提取矩形的四个顶点 (x1, y1), (x2, y2), (x3, y3), (x4, y4)
    x_rect = x[:, :4]  # (num, 4)
    y_rect = y[:, :4]  # (num, 4)

    # 提取关键点 (x_keypoint, y_keypoints)
    x_keypoint = x[:, 4]  # (num,)
    y_keypoint = y[:, 4]  # (num,)

    # 计算矩形的边界 (xmin, xmax, ymin, ymax)
    x_max = np.amax(x_rect, axis=1)  # 每行 x 坐标的最大值 (num,)
    x_min = np.amin(x_rect, axis=1)  # 每行 x 坐标的最小值 (num,)
    y_max = np.amax(y_rect, axis=1)  # 每行 y 坐标的最大值 (num,)
    y_min = np.amin(y_rect, axis=1)  # 每行 y 坐标的最小值 (num,)

    # 计算矩形的中心点 (center_x, center_y)
    x_ctr = (x_max + x_min) / 2.0  # 中心点 x 坐标 (num,)
    y_ctr = (y_max + y_min) / 2.0  # 中心点 y 坐标 (num,)

    # 判断矩形中心是否在图像范围内
    keep_masks = (x_ctr > 0) & (x_ctr < w) & (y_ctr > 0) & (y_ctr < h)

    # 判断额外的点是否在图像范围内
    keep_masks &= (x_keypoint > 0) & (x_keypoint < w) & (y_keypoint > 0) & (y_keypoint < h)
