...
from utils.augmentations_keypoints import Albumentations, augment_hsv, copy_paste, letterbox, mixup, random_perspective
from utils.general import (LOGGER, NUM_THREADS, check_dataset, check_requirements, check_yaml, clean_str,
                           segments2boxes, xyn2xy, xywh2xyxy, xywhn2xyxy, xyxy2xywhn)
from utils.torch_utils import torch_distributed_zero_first
from utils.rboxs_utils import poly_filter, poly2rbox, poly_and_keypoint_filter
...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            # 缩放后的图像、原图大小、缩放后的大小、图像的缩放后的标签（除了类别，坐标被缩放）
            ...
            if labels.size:  
                ...
                # keypoints
                labels[:, [1, 3, 5, 7, 9]] = img_label[:, [1, 3, 5, 7, 9]] * ratio[0] + pad[0]
                labels[:, [2, 4, 6, 8, 10]] = img_label[:, [2, 4, 6, 8, 10]] * ratio[1] + pad[1]
        ...
        # *[clsid poly] to *[clsid cx cy l s theta gaussian_θ_labels] θ∈[-pi/2, pi/2) non-normalized
            rboxes, csl_labels  = poly2rbox(polys=labels[:, 1:9], 
            ...
            labels_obb = np.concatenate((labels[:, :1], rboxes, labels[:, 9:11],csl_labels), axis=1) # keypoints
            # 筛选过滤掉比较小的框
        ...
        if hyp:
            c_num = 7 + hyp['cls_theta'] + 2 # [index_of_batch clsid cx cy l s theta gaussian_θ_labels, keypoint_x, kypoint_y]
        else:
            c_num = 187 + 2 # keypoints
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if labels.size:
            ...
            # keypoints
            labels[:, [1, 3, 5, 7, 9]] = img_label[:, [1, 3, 5, 7, 9]] + padw
            labels[:, [2, 4, 6, 8, 10]] = img_label[:, [2, 4, 6, 8, 10]] + padh
    ...
    labels_mask = poly_and_keypoint_filter(polys_and_keypoints=labels4[:, 1:].copy(), h=h_filter, w=w_filter) # keypoints
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                for label in labels:
                    ...
                    cls_id = cls_name_list.index(label[10]) # keypoints
                    # l_.append(np.concatenate((cls_id, label[:8]), axis=None))
                    l_.append(np.concatenate((cls_id, label[:10]), axis=None)) # keypoints
            ...
            if nl:
                ...
                assert len(label) == 12, f'Yolov5-OBB labels require 12 columns, which same as DOTA Dataset and add keypoints, {len(label)} columns detected' # keypoints
                ...
                l = np.zeros((0, 11), dtype=np.float32) # keypoints
        else:
            nm = 1  # label missing
            # l = np.zeros((0, 5), dtype=np.float32)
            # l = np.zeros((0, 9), dtype=np.float32)
