...
def xyxy_point2xywh_point(x):
    # Convert n x 6 boxes from [x1, y1, x2, y2, point_x, point_y] to [x, y, w, h, point_x, point_y] where xy1=top-left, xy2=bottom-right
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[:, 0] = (x[:, 0] + x[:, 2]) / 2  # x center
    y[:, 1] = (x[:, 1] + x[:, 3]) / 2  # y center
    y[:, 2] = x[:, 2] - x[:, 0]  # width
...
def scale_polys_and_points(img1_shape, polys_and_points, img0_shape, ratio_pad=None):
    # ratio_pad: [(h_raw, w_raw), (hw_ratios, wh_paddings)]
    # Rescale coords (xyxyxyxy) from img1_shape to img0_shape
    if ratio_pad is None:  # calculate from img0_shape
        gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain  = resized / raw
    ...
    polys_and_points[:, [0, 2, 4, 6, 8]] -= pad[0]  # x padding
    polys_and_points[:, [1, 3, 5, 7, 9]] -= pad[1]  # y padding
    polys_and_points[:, :10] /= gain # Rescale poly shape to img0_shape
    #clip_polys(polys, img0_shape)
    return polys_and_points

def scale_points(img1_shape, points, img0_shape, ratio_pad=None):
    # ratio_pad: [(h_raw, w_raw), (hw_ratios, wh_paddings)]
    # Rescale coords (xyxyxyxy) from img1_shape to img0_shape
    if ratio_pad is None:  # calculate from img0_shape
        gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain  = resized / raw
    ...
    points[:, [0]] -= pad[0]  # x padding
    points[:, [1]] -= pad[1]  # y padding
    points[:, :2] /= gain # Rescale poly shape to img0_shape
    #clip_polys(polys, img0_shape)
    return points
...
def non_max_suppression_obb_and_keypoint(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                                         labels=(), max_det=1500):
    """Runs Non-Maximum Suppression (NMS) on inference results_obb and keypoints
    Args:
    ...
    nc = prediction.shape[2] - 5 - 180 - 2  # number of classes
    xc = prediction[..., 4] > conf_thres  # candidates
    class_index = nc + 7
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x[xc[xi]]  # confidence, (tensor): (n_conf_thres, [cx cy l s obj point_x, point_y, num_cls theta_cls])

        # Cat apriori labels if autolabelling (由于当前不需要，未做修改)
        if labels and len(labels[xi]):
            l = labels[xi]
            v = torch.zeros((len(l), nc + 5), device=x.device)
            v[:, :4] = l[:, 1:5]  # box
        ...
        # Compute conf x[:, 4]是指包含物体的置信度
        x[:, 7:class_index] *= x[:, 4:5]  # conf = obj_conf * cls_conf

        _, theta_pred = torch.max(x[:, class_index:], 1,  keepdim=True) # [n_conf_thres, 1] θ ∈ int[0, 179]
        theta_pred = (theta_pred - 90) / 180 * pi # [n_conf_thres, 1] θ ∈ [-pi/2, pi/2)

        # Detections matrix nx9 (xyls, θ, point_x, point_y, conf, cls) θ ∈ [-pi/2, pi/2)
        if multi_label:
            i, j = (x[:, 7:class_index] > conf_thres).nonzero(as_tuple=False).T # ()
            x = torch.cat((x[i, :4], theta_pred[i], x[i, 5:7], x[i, j + 7, None], j[:, None].float()), 1)
        else:  # best class only
            conf, j = x[:, 7:class_index].max(1, keepdim=True)
            x = torch.cat((x[:, :4], theta_pred, x[:, 5:7], conf, j.float()), 1)[conf.view(-1) > conf_thres]
        ...
        if not n:  # no boxes
            ...
            x = x[x[:, 7].argsort(descending=True)[:max_nms]]  # sort by confidence
