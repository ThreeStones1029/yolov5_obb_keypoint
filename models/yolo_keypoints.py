...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.no = nc + 5 + 2 + 180  # number of outputs per anchor : # [class number + bbox + angle + keypoint (k_x, k_y)]
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                y = y + torch.cat((x[i][:, :, :, :, 0:5].sigmoid(), torch.cat((x[i][:, :, :, :, 5:7], x[i][:, :, :, :, 7:].sigmoid()), 4)), 4)
                if self.inplace:
                    y[..., 0:2] = (y[..., 0:2] * 2 - 0.5 + self.grid[i]) * self.stride[i]  # xy
                    y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh

                    # Keypoint prediction（增加关键点预测）
                    y[..., 5:7] =  y[..., 5:7] * self.anchor_grid[i] + self.grid[i].to(x[i].device) * self.stride[i]  # landmark x y

                else:  # for YOLOv5 on AWS Inferentia https://github.com/ultralytics/yolov5/pull/2953
                    xy = (y[..., 0:2] * 2 - 0.5 + self.grid[i]) * self.stride[i]  # xy
                    wh = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                    # Keypoint prediction（增加关键点预测）
                    landmark =  y[..., 5:7] * self.anchor_grid[i] + self.grid[i].to(x[i].device) * self.stride[i]  # landmark x y
                    y = torch.cat((xy, wh, y[..., 4:5], landmark, y[..., 7:]), -1)  # Concatenate keypoint and other outputs
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    no = na * (nc + 185 + 2)  # number of outputs = anchors * (classes + 185 + 2)
